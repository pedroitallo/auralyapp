...
const navigationItems = [
  ...
  { title: "Reading", url: createPageUrl("reading"), icon: BookHeart, target: "bottom-nav-reading" },
  { title: "Love Map", url: createPageUrl("journeys"), icon: Map, target: "bottom-nav-journey" },
  { title: "Guides", url: createPageUrl("guides"), icon: BookOpen, target: "bottom-nav-guides" },
  { title: "Profile", url: createPageUrl("profile"), icon: User, target: "bottom-nav-profile" },
];
...
export default function Layout({ children, currentPageName }) {
  ...
  if (isNoMenuPage() || (currentPageName && (currentPageName.toLowerCase() === 'chat' || currentPageName.toLowerCase() === 'chats'))) {
    return (
      <MixpanelProvider>
              ...
              <div className="flex justify-around items-center">
                {navigationItems.map((item) => {
                  const isActive = location.pathname === item.url || 
                                   (item.title === 'Love Map' && location.pathname.startsWith('/journeys')) || 
                                   (item.title === 'Reading' && location.pathname.startsWith('/reading'));
                  return (
                    <Link
                      key={item.title}
                      to={item.url}
