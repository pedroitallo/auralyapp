...
const SupportCard = ({ icon: Icon, title, description, actionType, target }) => {
  ...
  const handleClick = () => {
    if (title === 'Help & Support') {
      track('Support_Contact_Clicked');
    }
    if (title === 'Manage Payments') {
      track('ManagePayments_Accessed');
    }
  };
  ...
  if (actionType === 'navigate') {
    return <Link to={target} onClick={handleClick}><CardContent /></Link>;
  }

  return <a href={target} target="_blank" rel="noopener noreferrer" onClick={handleClick}><CardContent /></a>;
};
